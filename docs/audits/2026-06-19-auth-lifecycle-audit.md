...
This branch (`claude/auth-lifecycle-fixes`) lands the **A1 reliability cluster** and the **MFA gate cluster**. Everything else remains open and is tracked here for triage.
...
| ✅ **Fixed** | **C1, C2, H1** — MFA gate: AAL is recomputed on every session establishment (boot + `onAuthStateChange`, incl. `TOKEN_REFRESHED`) and cleared on sign-out; `ProtectedRoute` renders `MFAChallenge` when `mfaPending`; `needsMFAVerification` fails **closed**. | `AuthContext.tsx`, `ProtectedRoute.tsx`, `mfaService.ts` |
| ◑ **Mitigated** | **H2** — the *bypass* is closed by C2 (the destination route re-gates on `mfaPending`, so a premature redirect can no longer skip the challenge). The residual is cosmetic only (a possible brief render before the local AAL check settles); the audit's tri-state polish remains open. | via C2 |
| ⏳ **Open — own design** | **C3, C4, C5** (portal identity / `anon` reads / `change_portal_password`) — require the signed-JWT + `TO portal` RLS design in `docs/portal-identity-design.md`. **Do not** band-aid by opening tables to `anon`. C5 is independently exploitable and should be fixed regardless. | — |
| ⏳ **Open — needs server work** | C1 server backstop (`aal2` RLS), **H3, H4, H5, H6, M1** — token-storage hardening, expiry UX, and server enforcement of password-reset / permission-freshness / deactivation. | — |
| ⏳ **Open — triage** | **M2–M5, M7–M15, L1–L8** | — |
