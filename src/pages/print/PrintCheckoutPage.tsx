...
import { generateCheckoutForm } from '../../lib/pdf/pdfService';
import { Printer, X, Loader2, AlertCircle, RefreshCw } from 'lucide-react';

export const PrintCheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    const generatePDF = async () => {
      ...
      const result = await generateCheckoutForm(caseId, false);
  ...
  const handleRetry = () => {
    if (caseId) {
      setIsGenerating(true);
      setError(null);
      generateCheckoutForm(caseId, false).then((result) => {
        if (!result.success) {
          setError(result.error || 'Failed to generate PDF');
        }
        setIsGenerating(false);
  ...
  const handleDownload = () => {
    if (caseId) {
      generateCheckoutForm(caseId, true);
    }
  };
