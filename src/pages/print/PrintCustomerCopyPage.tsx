...
import { generateCustomerCopy } from '../../lib/pdf/pdfService';
import { Printer, X, Loader2, AlertCircle, RefreshCw } from 'lucide-react';

export const PrintCustomerCopyPage = () => {
  ...
  useEffect(() => {
    ...
    const generatePDF = async () => {
      ...
      const result = await generateCustomerCopy(caseId, false);
  ...
  const handleRetry = () => {
    if (caseId) {
      setIsGenerating(true);
      setError(null);
      generateCustomerCopy(caseId, false).then((result) => {
        if (!result.success) {
          setError(result.error || 'Failed to generate PDF');
        }
        setIsGenerating(false);
  ...
  const handleDownload = () => {
    if (caseId) {
      generateCustomerCopy(caseId, true);
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-lg p-8 max-w-md text-center">
        {isGenerating ? (
          <>
            ...
            <p className="text-slate-600">Please wait while your customer copy is being generated...</p>
          </>
        ) : error ? (
          <>
            <AlertCircle className="w-16 h-16 text-danger mx-auto mb-4" />
          ...
          <>
            ...
            <p className="text-slate-600 mb-6">Your customer copy has been generated and opened.</p>
            <div className="flex gap-3 justify-center">
              <button
...
export default PrintCustomerCopyPage;
